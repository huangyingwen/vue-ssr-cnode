  ...
  <transition name="slide-down">
    <header class="app-header-wrapper" v-show="show">
      ...
      <div class="app-header-right">
        <slot name="actions">
          <v-btn icon dark v-for="(action, actionIdx) in actions" :key="actionIdx" @click.native="handleClick('action', {actionIdx, route: action.route})">
      ...
      <!-- <v-toolbar dark color="primary text-xs-left">
        <v-toolbar-side-icon v-if="showMenu" @click.native="handleClick('menu')"></v-toolbar-side-icon>
        <v-btn icon v-if="showBack" @click.native="handleClick('back')">
          <v-icon>arrow_back</v-icon>
        </v-btn>
        <v-toolbar-title class="text-xs-left white--text">{{ title }}</v-toolbar-title>
        <v-spacer></v-spacer>
        <slot name="actions">
          <v-btn icon dark v-for="(action, actionIdx) in actions" :key="actionIdx" @click.native="handleClick('action', {actionIdx, route: action.route})">
            <icon v-if="action.svg" :name="action.svg" class="app-header-icon"></icon>
...
export default {
  ...
  computed: {
    ...
    ...mapState('appShell', ['isPageSwitching'])
  },
  methods: {
    /**
     * 处理按钮点击事件
