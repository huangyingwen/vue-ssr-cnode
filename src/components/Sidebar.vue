<template>
  <div class="sidebar-wrapper" ref="sidebarWrapper" :class="wrapperClass">
    <div class="sidebar-scroller" ref="sidebarScroller" :style="{
                'padding-left': widthProp
            }">
      <div class="sidebar-main" :style="{
                    width: widthProp
                }" @scroll.stop>
        <slot></slot>
      </div>
      <div class="touch-toggle" :style="{
                    'opacity': opacity,
                    'padding-left': widthProp
                }" ref="sidebarToggle" @click.stop.prevent="toggleClick"></div>
    </div>
  </div>
</template>
...
if (process.env.VUE_ENV === 'client') {
  rAF =
    window.requestAnimationFrame || window.webkitRequestAnimationFrame || rAF
}
...
export default {
  ...
  computed: {
    ...
    zone() {
      ...
      return {
        top: top === undefined ? clientHeight - bottom - height : top,
        left: left === undefined ? clientWidth - right - width : left,
        width: width === undefined ? clientWidth - left - right : width,
        height: height === undefined ? clientHeight - top - left : height
      }
    }
  },
  watch: {
