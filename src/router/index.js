...
function route(path, name, view = name) {
  return {
    path: path,
    name: name,
  ...
  router.beforeEach((to, from, next) => {
    if (router.app.$store) {
      ...
      if (router.app.$store.state.appShell.needPageTransition) {
        ...
        router.app.$store.commit(`appShell/${types.SET_PAGE_TRANSITION_NAME}`, {
          pageTransitionName
        })
      }
    }
 ...
 * @const
 */
const ALWAYS_BACK_PAGE = ['home', 'topics']
